...
function Contact({
  ...
  return (
    <Section
      title="연락처 및 마음 전하실 곳"
      className={cx('contents__contact')}
    >
      <Accordion label="신랑측">
        <ContactInfo
          name={groom.name}
...
function ContactInfo({ name, account, phoneNumber }: Person) {
  return (
    <div className={cx('contact_info')}>
      {/* 정보 표현 */}
      <div className={cx('contact_info--person')}>
        <span>{`${account.bankName} | ${account.accountNumber}`}</span>
        <span>{name}</span>
      </div>
      {/* 버튼들 */}
      <ul className={cx('contact_info--btns')}>
        <li>
          <a className={cx('link')} href={`tel: ${phoneNumber}`}>
            전화
          </a>
        </li>
        <li>
